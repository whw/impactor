...
from tSrv.DataService.tCass.cqlshlib import cql3handling, cqlhandling, pylexotron, sslhandling
from tSrv.DataService.tCass.cqlshlib.copyutil import ExportTask, ImportTask
from tSrv.DataService.tCass.cqlshlib.displaying import (ANSI_RESET, BLUE, COLUMN_NAME_COLORS, CYAN,
                                 RED, WHITE, FormattedValue, colorme)
from tSrv.DataService.tCass.cqlshlib.formatting import (DEFAULT_DATE_FORMAT, DEFAULT_NANOTIME_FORMAT,
                                 DEFAULT_TIMESTAMP_FORMAT, CqlType, DateTimeFormat,
                                 format_by_type, formatter_for)
from tSrv.DataService.tCass.cqlshlib.tracing import print_trace, print_trace_session
from tSrv.DataService.tCass.cqlshlib.util import get_file_encoding_bomsize, trim_if_present
...
(options, arguments) = parser.parse_args([], values=optvalues)
...
def format_value(val, cqltype, encoding, addcolor=True, date_time_format=None,
...
class Shell(cmd.Cmd):
    ...
    def __init__(self, hostname, port, color=True,
...
def make_shell(options=None, hostname=None, port=None):
    if options is None:
        options, hostname, port = read_options([], os.environ) 
...
def main(options, hostname, port):
    shell = make_shell(options, hostname, port)
    shell.cmdloop()
    save_history()
...
if __name__ == '__main__':
    opts =read_options(sys.argv[1:], os.environ)
    print opts
    main(*opts)
